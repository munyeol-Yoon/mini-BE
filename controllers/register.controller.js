...
const jwtValidation = require("../middleware/auth-middleware"); // 해당 라우터 validation 적용
const {
  signupSchema,
  loginSchema,
...
const signupController = async (req, res) => {
  try {
    ...
    if (existUsername) {
      return res.status(412).json({ errorMessage: "이미 가입된 닉네임입니다." });
    }

    // password double security
    const hashedPassword = bcrypt.hashSync(password, 6);
    const signUp = await Users.create({ // / =new Users()
      name,
...
const loginController = async (req, res) => {
  try {
    ...
    if (!user || !bcrypt.compareSync(password, user.password)) { 
      ...
      return res.status(412).json({ errorMessage: "이름 또는 패스워드를 확인해주세요." });
    }
    console.log(user)
